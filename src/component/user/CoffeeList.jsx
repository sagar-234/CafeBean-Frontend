...
const CoffeeList=()=>{
  ...
  setCoffeeItem(updatedCoffeeitem)
    ...
    const deleteCoffeeItem=(item)=>{
      if(window.confirm("Are you sure to delete the product "+ item.coffee.coffee_type))
      {
      const updatedCoffeeitem = [...coffeeitem];
    ...
    const getCoffeeItem=(userid)=>{
      CoffeeItemService.getCoffeeItemById(userid).then(
        res=>{
        ...
        <div key={t.tableId} className="col-lg-3 col-md-4 mb-4" >
            <div className="card h-100" >
              <div className="card-body " >
                <h6 className="card-title">Name:{t.coffee_type}</h6>
                <p className="text-muted">Price :{t.price}</p>
              </div>
              <div className="card-footer">
                <button className="btn btn-lg btn-block" onClick={()=>AddCoffee(JSON.parse(sessionStorage.getItem('log')).cust_id,t)} >Add Coffee</button>
              </div>
            </div>
          </div>
...
<button className="btn btn-primary btn-lg" onClick={modalShow}>Next</button>
...
<Modal centered show={show} onHide={modalClose}>  
  ...
  <Modal.Body>  
   {
    <ul className="list-group" style={{paddingTop:'20px',paddingLeft:'20px',paddingRight:'20px',overflowY:'scroll'}}>
    {coffeeitem.map((item) => (
      <li key={item.coffeeItemId} className="list-group-item cart-item" >
        <div className="row align-items-center">
          
          <div className="col-md-9">
            <h3 className="cart-item-name">{item.coffee.coffee_type}</h3>
            <p className="cart-item-price">Price: {item.coffee.price}</p>
            ...
            <button className="btn add btn-sm" onClick={()=>{deleteCoffeeItem(item)}}><BsFillTrashFill/></button>
          </div>
        </div>
      </li>
    ))}
  ...
  <Modal.Footer>  
    <button className="btn" style={{backgroundColor: '#fc8019'}} disabled={coffeeitem.length===0} onClick={confirmOrder}>Confirm</button>
